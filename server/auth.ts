...
export async function createGuestUser() {
  const guestTag = Date.now().toString(36) + Math.random().toString(36).substr(2, 4);
...
export async function findOrCreateSocialUser(
  ...
  let username = displayName.replace(/[^a-zA-Z0-9_]/g, "").slice(0, 20);
  if (username.length < 3) {
    username = `${provider}_${Date.now().toString(36)}`;
  }
