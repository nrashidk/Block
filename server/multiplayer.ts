...
import { db } from "./db";
import { users, matches, matchQueue } from "@shared/schema";
...
function createBotPlayer(targetElo: number): PlayerSocket {
  ...
  return {
    ws: mockWs,
    userId: `bot_${Date.now()}_${Math.random().toString(36).substr(2, 6)}`,
...
function findMatch(): [PlayerSocket, PlayerSocket] | null {
  ...
  for (let i = 0; i < queue.length; i++) {
    for (let j = i + 1; j < queue.length; j++) {
      ...
      const waitI =
        (Date.now() -
          (connectedPlayers.get(queue[i].userId)
            ? Date.now()
            : Date.now())) /
        1000;
  ...
  switch (data.type) {
    ...
    case "score_update": {
      ...
      if (match.player1.userId === player.userId) {
        match.player1Score = data.score;
        match.player1Pieces = data.piecesPlaced || 0;
        sendMessage(match.player2.ws, "opponent_update", {
          score: data.score,
          piecesPlaced: data.piecesPlaced,
        });
      } else if (match.player2.userId === player.userId) {
        match.player2Score = data.score;
        match.player2Pieces = data.piecesPlaced || 0;
        sendMessage(match.player1.ws, "opponent_update", {
          score: data.score,
          piecesPlaced: data.piecesPlaced,
        });
