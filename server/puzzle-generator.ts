import { GRID_SIZE, BLOCK_SHAPES } from "../lib/game-types";
...
const PIECE_SHAPES: number[][][] = BLOCK_SHAPES.map((shape) =>
  shape.map((row) => row.map((cell) => (cell ? 1 : 0)))
);
...
export function generateTargetPuzzles() {
  ...
  const chapters = [
    ...
    { difficulty: "easy", fillRate: 0.20, moves: 7, baseScore: 150 },
    { difficulty: "medium", fillRate: 0.28, moves: 7, baseScore: 220 },
    { difficulty: "medium", fillRate: 0.35, moves: 6, baseScore: 300 },
    { difficulty: "hard", fillRate: 0.42, moves: 5, baseScore: 400 },
    { difficulty: "hard", fillRate: 0.50, moves: 5, baseScore: 520 },
  ];
  ...
  for (let ch = 0; ch < chapters.length; ch++) {
    ...
    const published = ch < 3;
    for (let lvl = 1; lvl <= LEVELS_PER_CHAPTER; lvl++) {
      const seed = `target_ch${ch + 1}_lvl${lvl}`;
      const rng = createSeededRandom(seed);

      const grid: { filled: boolean; colorIndex: number }[][] = [];
      const adjustedFill = config.fillRate + (lvl - 1) * 0.012;
      for (let r = 0; r < GRID_SIZE; r++) {
        const row = [];
        for (let c = 0; c < GRID_SIZE; c++) {
      ...
      const pieces = [];
      const movesLimit = Math.max(3, config.moves - Math.floor(lvl / 5));
      ...
      puzzles.push({
        ...
        gridSetup: grid,
        pieceSequence: pieces,
        targetScore: config.baseScore + lvl * 25,
        movesLimit,
